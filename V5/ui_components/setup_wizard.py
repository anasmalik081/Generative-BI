...
class SetupWizard:
    ...
    def _step_semantic_setup(self):
        ...
        existing_info = enhanced_semantic_layer.get_existing_collection_info("default")
        
        if existing_info["exists"]:
            st.success(f"✅ Found existing semantic layer with {existing_info['count']} embeddings")
            st.info(f"📋 Tables covered: {', '.join(existing_info['tables'][:5])}{'...' if len(existing_info['tables']) > 5 else ''}")
            
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🚀 Use Existing Semantic Layer", type="primary", use_container_width=True):
                    st.session_state.semantic_layer_built = True
                    st.session_state.semantic_method = "Existing"
                    st.session_state.wizard_step = 3
                    st.rerun()
            
            with col2:
                if st.button("🔄 Rebuild Semantic Layer", use_container_width=True):
                    st.session_state.force_rebuild_semantic = True
                    st.rerun()
            
            # Only show rebuild options if user chose to rebuild
            if not st.session_state.get('force_rebuild_semantic', False):
                return
            else:
                st.warning("⚠️ Rebuilding will replace existing embeddings and consume API credits")
    ...
    def _execute_automatic_semantic_generation(self, business_domain: str, include_examples: bool):
        ...
        try:
            ...
            enhanced_semantic_layer.create_enhanced_schema_embeddings(
                ...
                force_rebuild=st.session_state.get('force_rebuild_semantic', False)
            )
            ...
            st.session_state.wizard_step = 3
            if 'force_rebuild_semantic' in st.session_state:
                del st.session_state.force_rebuild_semantic
        ...
        try:
            with st.spinner("🏗️ Building semantic layer from your metadata..."):
                ...
                enhanced_semantic_layer.create_enhanced_schema_embeddings(
                    ...
                    sample_queries=[],
                    connection_name="default",
                    force_rebuild=True  # Manual build always rebuilds
                )
        ...
        try:
            with st.spinner("🔄 Building hybrid semantic layer..."):
                ...
                enhanced_semantic_layer.create_enhanced_schema_embeddings(
                    ...
                    sample_queries=[],
                    connection_name="default",
                    force_rebuild=True  # Hybrid build always rebuilds
                )
