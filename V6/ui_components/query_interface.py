    ...
    def _render_results(self):
        ...
        if result.get('error_message'):
            ...
            if result.get('generated_sql'):
                with st.expander("🔍 Generated SQL (Blocked)"):
                    st.code(result['generated_sql'], language='sql')
            
            # Show suggestions for authorized queries
            st.info("💡 **Suggestion:** Try queries with data you have access to, or contact your administrator for additional permissions.")
            return
                ...
                if len(categorical_cols) > 0:
                    ...
                    for col in categorical_cols[:3]:
                        try:
                            ...
                            st.write(f"**{col}:** Unable to calculate unique values")
        else:
